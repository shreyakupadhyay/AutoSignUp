...
Description:
Using a fake mail server and getting email content from the fake mail server to get the link to confirmation email of the gradbuster website.
'''
import logging
import requests
import re
...
def getMailContent(link):
	...
	for link in soup.findAll('a', href=True, text={'Confirm Email'}):
		authLink = link["href"]
	#emailResponse = requests.get(authLink)

	


getMailLink("Purs1959","armyspy.com")
